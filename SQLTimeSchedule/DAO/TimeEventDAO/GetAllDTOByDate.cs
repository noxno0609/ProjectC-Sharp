...
namespace SQLTS
{
    ...
    {
        public static List<TimeEventDTO> GetAllDTOByDate(DateTime date, MySqlConnection conn)
        {
            List<TimeEventDTO> listdto = new List<TimeEventDTO>();

            conn.Open();
            string sql = "SELECT * FROM timeevent WHERE Dayselect = '" + SQLFormat.addDateTime(date) + "'";
